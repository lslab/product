 ...
 *
 * <p>self can be used for form and Model data validation before saving/inserting/deleting a data.</p>
 *
 * <p>To use self, you have to create an instance of it and defined the validation rules.
 ...
 * </code>
 *
 * <p>You can get the list of available validation rules by calling self::getAvailableRules()</p>
 *
 * <p>To validate the data, create an instance of self and call validate() in your Controller/Model.</p>
 * <code>
 * $v = new self();
 *
 * # There are 3 different validation Mode.
 * //$v->checkMode = self::CHECK_SKIP;
 * //$v->checkMode = self::CHECK_ALL_ONE;
 *
 ...
 *
 * @author Leng Sheng Hong <darkredz@gmail.com>
 * @version $Id: self.php 1000 2009-08-30 11:37:22
 * @package doo.helper
 * @since 1.2
 */
class Validator {
            ...
            foreach($cRule as $v2){
                if(is_array($v2)){
                    ...
                    if($err = call_user_func_array(array(&$this, 'test'.$v2[0]), $vv) ){
                        if($this->checkMode==self::CHECK_ALL)
                            $errors[$k][$v2[0]] = $err;
                        else if($this->checkMode==self::CHECK_SKIP && !empty($v) && $v2[0]!='optional'){
                            return $err;
                        }else if($this->checkMode==self::CHECK_ALL_ONE)
                            $errors[$k] = $err;
                    }
                    ...
                    if(sizeof($cRule)>1){
                        ...
                        if($err = call_user_func_array(array(&$this, 'test'.$cRule[0]), $vv) ){
                            if($this->checkMode==self::CHECK_ALL || $this->checkMode==self::CHECK_ALL_ONE)
                                $errors[$k] = $err;
                            else if($this->checkMode==self::CHECK_SKIP)
                                return $err;
                        }
