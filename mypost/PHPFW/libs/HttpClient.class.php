...
//		}
	...
	public function post2($url, $form = array())
	{
		return $this->fetchNoResponse($url, 'POST', $this->_maxredirect, $form);	//modified by eyes
	}	

	
	private function fetchNoResponse($url, $method, $maxredir = null, $form = array(), $files = array())	//modifyed by eyes
	{
		...
		/* open connection */
		//if (!$this->sockHandle) {
			$this->sockHandle = @pfsockopen(($scheme=='https' ? "ssl://$host" : $host), $scheme == 'https' ? 443 : 80);
		//}
		...
		@fputs($this->sockHandle, $reqhead.$reqbody);
//		@fputs($this->sockHandle, "\r\n\r\n");
		//for($res=''; !feof($fp); $res.=@fgets($fp, 4096)) {}
		fclose($this->sockHandle);
